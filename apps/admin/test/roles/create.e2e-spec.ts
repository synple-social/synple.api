...
describe('POST /roles', () => {
  ...
  describe('Nominal case', () => {
    ...
    it('Returns a 201 (Created) status code and the correct body', () => {
      ...
      expect(response.body).toEqual({ name: 'Test role', isDefault: true, uuid: TEST_UUID })
    })
  ...
  describe('Alternative case : creating two default roles', () => {
    ...
    it('Has made the first role not default', async () => {
      expect(await model.count({ where: { name: 'First role', isDefault: false } })).toEqual(1)
    })
    it('Has made the second role a default role', async () => {
      expect(await model.count({ where: { name: 'Second role', isDefault: true } })).toEqual(1)
    })
  })
