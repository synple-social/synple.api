...
describe('DELETE /roles/:uuid/scopes/scopeId', () => {
  ...
  beforeAll(async () => {
    ...
    role = await app.get('RoleRepository').create({ name: 'test role', uuid: '1', isDefault: false })
    scope = await app.get('ScopeRepository').create({ slug: 'test::scope', uuid: "1" })
  })
  ...
  describe('Nominal case', () => {
    ...
    beforeAll(async () => {
    await app.get('RoleScopeRepository').create({ roleId: role.id, scopeId: scope.id })
  ...
  describe('Exception case : the role is not found', () => {
    ...
    afterAll(async () => await app.get('RoleScopeRepository').destroy({ where: { }}))
