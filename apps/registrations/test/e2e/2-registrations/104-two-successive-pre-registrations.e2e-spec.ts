...
async function getLastConfirmationCode(email: string, repository: typeof PreRegistration): Promise<string> {
  const preRegistration = await repository.findOne({ where: { email }, order: [['createdAt', 'DESC']] })
