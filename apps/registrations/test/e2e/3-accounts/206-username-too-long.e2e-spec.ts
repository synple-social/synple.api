...
describe('Pre-registrations scenarios', () => {
  ...
  describe('[SC-205] the username is under six characters long', () => {
    ...
    beforeAll(async () => {
      ...
      const username = new Array(258).join('a')
      response = createAccount({ email, registrationId, password: 'a', passwordConfirmation: 'a', username }, app)
    })
    ...
    it('Returns a 400 (Bad Request) status code with the correct body', () => {
      return response
        ...
        .expect({ path: 'username', error: 'maxlength' })
    })
