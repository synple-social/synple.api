...
describe('Pre-registrations scenarios', () => {
  ...
  describe('[SC-205] the username is under six characters long', () => {
    ...
    beforeAll(async () => {
      ...
      const firstPreRegistration = await models.preRegistrations.findOne({ where: { email } })
    ...
    it("Has created the first account but not the second one", async () => {
      expect((await models.accounts?.findOne({ where: { username: 'testUser' } }))?.dataValues.email).toEqual(email)
    })
  })
})
