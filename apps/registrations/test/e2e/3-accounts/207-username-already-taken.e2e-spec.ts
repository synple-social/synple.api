...
describe('Pre-registrations scenarios', () => {
  ...
  describe('[SC-205] the username is under six characters long', () => {
    ...
    beforeAll(async () => {
      ...
      await createAccount({ email, registrationId, password: 'a', passwordConfirmation: 'a', username }, app)

      await createPreregistration('other@mail.com', app)
      const preRegistration = await models.preRegistrations.findOne({ email: 'other@mail.com' }) as PreRegistration
      await createRegistration('other@mail.com', `${preRegistration.confirmationCode}`, app)
      const { id } = (await models.registrations.findOne({ email: 'other@mail.com' })) as RegistrationDocument
      response = createAccount({ email: 'other@mail.com', registrationId: id, password: 'a', passwordConfirmation: 'a', username }, app)
    })
    ...
    it("Has created one account in the database", async () => {
      expect(await models.accounts?.countDocuments()).toBe(1)
    })
    it("Has created the first account but not the second one", async () => {
      expect((await models.accounts?.findOne({ username: 'testUser' }))?.email).toEqual(email)
    })
  })
})
