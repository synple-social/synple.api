...
describe("Accounts scenarios", () => {
	...
	describe("[SC-207] the username is already used by another user", () => {
		let response: any;
		let models: {
		...
		beforeAll(async () => {
			...
			const firstPreRegistration = await models.preRegistrations.findOne({
				where: { email },
			});
			await createRegistration(
				email,
				`${firstPreRegistration?.getDataValue("confirmationCode")}`,
				app,
			);
