...
describe("Password reset scenarios", () => {
  ...
  describe('[SC-404] a user attemps to modify iots password but failed the confirmation', () => {
    let response: any;
    ...
    beforeAll(async () => {
      ...
      await request(app.getHttpServer())
        ...
        .send({ email })
      const confirmationCode = (await model.findOne({ where: { email } }))?.confirmationCode
      response = request(app.getHttpServer())
        .post('/passwords/reset')
        .set('Accept', 'application/json')
        .send({ email, confirmationCode, password: 'newPassword', passwordConfirmation: 'otherPassword' })
    })
