...
describe("Password reset scenarios", () => {
  ...
  describe('[PWR-006] the user did not provide a confirmation code', () => {
    let response: any;
    ...
    beforeAll(async () => {
      ...
      response = request(app.getHttpServer())
        ...
        .set('Accept', 'application/json')
        .send({ email, password: 'newPassword', passwordConfirmation: 'newPassword' })
    })
    ...
    it('Returns a 204 (No Content) status code with the correct body', () => {
      return response
        ...
        .expect({ path: 'confirmationCode', error: 'required' })
    })
  })
})
